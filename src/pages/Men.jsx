...
import {
  FaBars, FaEnvelope, FaHeart, FaPlus, FaSearch, FaShoppingCart, FaTimes,
  FaFacebook, FaInstagram, FaWhatsapp, FaTwitter
} from 'react-icons/fa';
...
export default function Men() {
  ...
  const maleCategorySlugs = ["shirts", "trousers", "footwears", "watches"];

  // Fetch categories and featured collections
  useEffect(() => {
    const fetchCategories = async () => {
      setLoadingCategories(true);
      try {
        const data = await Promise.all(
          maleCategorySlugs.map(slug =>
            axios.get(`${BASE_URL}/categories/slug/${slug}`).then(res => res.data)
          )
        );
    ...
    const fetchFeatured = async () => {
      ...
      try {
        const { data } = await axios.get(`${BASE_URL}/collections/featured`, { params: { limit: 4 } });
  ...
  const handleAddToCart = async (category) => {
    try {
      ...
      if (!category?.subcategories?.length) return alert("No products found in this category");

      const product = await axios.get(`${BASE_URL}/products/${category.subcategories[0].slug}`).then(res => res.data);
      ...
      await axios.post(`${BASE_URL}/cart/items`, payload, { headers: { Authorization: `Bearer ${token}` } });
      alert(`Added ${product.name} to cart!`);
    } catch (err) {
      console.error("Error adding to cart:", err);
      alert(err.response?.status === 401 ? "Session expired. Please log in again." : "Failed to add product to cart");
    }
  };
    ...
    <div className="min-h-screen overflow-x-hidden bg-white text-gray-900">
        ...
        <div className="h-96 w-full bg-center bg-cover" style={{ backgroundImage: "url('/collectionbg.png')" }}>
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
            <div className="text-center mt-10 max-w-6xl px-6">
              <p className="mt-4 tracking-[0.8em] uppercase text-white/90">Men</p>
              <h1 className="text-white text-xl sm:text-2xl md:text-3xl font-bold leading-tight">
                Discover premium menswear curated for modern gentlemen. Timeless classics and contemporary styles.
              </h1>
            </div>
          </div>
        </div>
      ...
      <section className="bg-gray-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-xs md:text-sm text-center mb-6">Explore our curated categories for men</p>
              ...
              {categories.length ? categories.map(cat => (
                <div key={cat.id} className="bg-white border rounded-lg overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <button onClick={() => handleAddToCart(cat)} className="text-xs md:text-sm mt-2 inline-block text-blue-600 hover:underline">
                      Shop now →
                    </button>
                  </div>
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <p className="text-xs md:text-sm text-center mb-6">Signature collections crafted for men</p>
        {loadingFeatured ? (
          <p className="text-center">Loading featured collections...</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {featuredCollections.length ? featuredCollections.map(fc => (
              <CollectionCard
                key={fc.id}
                image={fc.imageUrl || '/men.png'}
                tag={fc.name}
                title={fc.name}
                description={fc.description}
                itemsAvailable={fc.productCount || 0}
                slug={`/collections/${fc.slug}`}
              />
            )) : <p className="text-center col-span-full text-gray-500">No featured collections available.</p>}
          </div>
        )}
      </section>
...
function CollectionCard({ image, tag, title, description, itemsAvailable, slug }) {
  return (
    <article className="bg-white h-full rounded-lg overflow-hidden shadow-sm border">
      ...
      <div className="p-4">
        <span className="text-xs text-black bg-gray-400 px-2 py-1 rounded-lg inline-block">{tag}</span>
